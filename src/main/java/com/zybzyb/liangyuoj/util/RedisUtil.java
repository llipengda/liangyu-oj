...
public class RedisUtil {
    ...
    /**
     * 在Redis中设置一个带有可选过期时间的键值对。
     *
     * @param key        要在Redis中设置的键
     * @param value      要在Redis中设置的值
     * @param timeMillis 过期时间的毫秒数
     * @return 如果成功设置了值，则返回true；否则返回false
     */
    public boolean tset(String key, Object value, Long timeMillis) {
        ...
        try {
            if (expire_time > 0) {
                redisTemplate.opsForValue()
                    .set(key, value, expire_time, TimeUnit.MILLISECONDS);
            } else {
                set(key, value);
            }
            return true;
